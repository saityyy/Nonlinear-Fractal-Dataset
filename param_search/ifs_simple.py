...
class ifs_function():
    def __init__(self, prev_x, prev_y, nonlinear_flag):
        # previous (x, y)
        ...
        self.temp_proba = 0.0
        self.nonliear_flag = nonlinear_flag
        ...
        for i in range(iteration-1):  # python3.x
            for j in range(len(select_function)):
                if rand[i] <= select_function[j]:
                    ...
                    if self.nonliear_flag:
                        nonlinear_function = np.random.sample(functions_list)
                        next_x, next_y = nonlinear_function(next_x, next_y)
                    break
